...
void WINAPI ServiceMain(DWORD argc, LPWSTR *argv) {
    ...
    wcsncpy(logPath, exePath, MAX_PATH - 20);
    wcsrchr(logPath, L'\\')[0] = L'\0';
    wcscat(logPath, L"\\data\\service_wrapper.log");
    FILE *f = _wfopen(logPath, L"a");
    if (f) {
        fwprintf(f, L"[%lu] Trying to start: %s\n", GetCurrentProcessId(), exePath);
        fclose(f);
    }
    ...
    if (CreateProcessW(exePath, L" --hidden", NULL, NULL, FALSE,
        ...
        if (f) {
            ...
            fwprintf(f, L"[%lu] CreateProcess succeeded\n", GetCurrentProcessId());
            fclose(f);
        }
    } else {
        ...
        if (f) {
            f = _wfopen(logPath, L"a");
            fwprintf(f, L"[%lu] CreateProcess failed: %lu\n", GetCurrentProcessId(), err);
            fclose(f);
        }
    }
