...
void LaunchProcess(const wchar_t *name) {
    ...
    si.cb = sizeof(si);
    if (CreateProcessW(NULL, (LPWSTR)name, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi)) {
...
// ========== Main - Silent background service ==========
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow) {
    ...
    // Check for --hidden flag to suppress any window
    BOOL hiddenMode = FALSE;
    if (lpCmdLine) {
        // Skip whitespace and check for --hidden
        while (*lpCmdLine == L' ' || *lpCmdLine == L'\t') lpCmdLine++;
        if (wcsncmp(lpCmdLine, L"--hidden", 8) == 0 || lpCmdLine[0] == L'\0') {
            hiddenMode = TRUE;
        }
    }
    
    // Register window class anyway (needed for message loop)
    WNDCLASSEXW wc = {0};
    wc.cbSize = sizeof(wc);
    ...
    // Skip window creation - just use message loop directly
    ...
    // Main loop - if hidden, no message pump needed
    if (hiddenMode) {
        // Simple loop without message pump
        int tick = 0;
    ...
    while (running) {
        ...
        if (tick >= 1) {  // Check every interval
            ...
            // Reload config every reloadInterval
            if (tick >= reloadTick) {
                LoadConfig();
                LoadSettings();
                LoadConfig();
                ...
                if (_waccess(sigPath, 0) == 0) {
                    LoadConfig();
                    _wremove(sigPath);  // Remove signal file
                    Log(L"配置已重新加载");
                }
            }
        }
